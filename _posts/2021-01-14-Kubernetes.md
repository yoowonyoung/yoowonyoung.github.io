...
- 어플리케이션 환경 변수 관리
    ...
    - 시스템 리소스 요구사항과 제한
        ...
        - 리소스 쿼타
            - 네임스페이스별 리소스 제한
            - kind 부분에 ResourceQuota로 설정
            - 리미트레인지 같은건 리소스의 정책으로, 리소스를 제한하는거고 리소스 쿼타는 총 량을 제한하는것
            - 네임스페이스 내의 모든 컨테이너의 합이 ~ 를 넘지 않도록 -> 이런식
            - 리소스쿼타를 describe를 하면 얼마나 썻는지, 얼마나 남았는지를 쉽게 볼 수 있음
            - 리소스쿼타가 적용된 상태에서, 총량을 넘어가는 팟이 만들어지려고 하는경우 실패하면서 에러 메시지로 뭐가 어떻게 넘겼는지 볼 수 있음
    - 데몬셋
        - 쿠버네티스 전반에 걸쳐 시스템을 지원하기 위해 떠있는 컨테이너
        - 노드당 포드를 하나씩 배치해주기 떄문에 노드의 팟 관리용으로 쓰이기도함
            - 레플리카셋은 무작위거든
            - hostpath같은거 쓰면서 데몬셋을 많이 같이씀(노드의 리소르를 관리하기 위해)
            - 리소스 로깅용으로 데몬셋이 많이쓰임
        - 노드별로 하나씩 배치되기 때문에, 레플리카셋처럼 몇개만들지 지정안해도됨
        - 대표적으로 kube-proxy가 데몬셋으로 만든 쿠버네티스에서 사용하는 컨테이너
        - toleration(마스터에도 팟을 올리게 해주는 옵션)을 뺴면 다른 레플리카셋하고 거의 비슷함
    - 스태틱 포드
        - 마스터 노드에 존재
        - 내가 직접 만드는게 아니라 kubelet이 직접 실행하는 포드
        - kubelet이 직접 지정한 디렉토리에 yaml이 있으면 자동으로 추가됨
        - 각각의 노드에서 kubelet에 의해서 실행
        - 노드들을 삭제할때 apiserver를 통해서 실행되지 않은 스태틱 포드는 삭제 불가
        - Etcd, Apiserver, scheduler, controller 모두 스태틱 포드로 구성되있음
        - 유저가 직접 만들어주지 않더라도 kubelet이 만들어줌
            - 변경만 해도 kubelet이 알아서 내리고 다시 올림
        - kubectl로 실행하는게 아니라 yaml파일을 만들어서 특정 경로에 배치만 해두면됨
            - /etc/kubernetes/manifests
        - 실행할때 별다른 명령어는 필요 없고, 특정 경로에 yaml파일만 만들어주면 되고, 수정하는것도 수정하면 자동으로 반영됨
    - 수동 스케줄링
        - 매뉴얼로 포드를 원하는 위치에 배치하는 방식
        - 특정 노드에 종속적으로 팟을 배치해야 하는경우 수동으로 스케줄링 해야함
            - GPU같은걸 돌려야 하는 경우
            - 파일 입출력이 많아 SSD를 써야 하는경우
        - 방법은 간단, pod의 spec분에 nodeName을 적어주면 됨
        - NodeSelector를 사용하는 방법도 있음, node에 label을 해두고, 그 라벨들에 매핑하면됨
    - 멀티플 스케줄러
        - 기존의 스케줄러는 큐브시스템에 있는 하나임
        - 기존의 스케줄러가 맘에 들지 않는 경우 개선해서 쓸 수 있도록 스케줄러 커스터마이징 가능함
            - go언어로 되어있어서 기존 스케줄러도 내부 설정 보고 변경 가능
        - 쿠버네티스에서도 서비스 어카운트라는 개념이 있는데, 이 계정에 권한이 있어야 리소스가 얼마남았는지 그런게 파악이 가능하므로, 권한(클러스터 API등에서)을 줘야지 멀티플 스케줄러가 가능
        - 권한이 없는 스케줄러를 통해 팟을 띄우면 펜딩됨, 클러스터롤 & 롤 바인딩에 추가해야지 활성화가됨
        - 팟의 스펙 부분에 스케줄러 네임을 지정 해줌으로써 권한 부여가능
    - 오토스케일링
        - 말 그대로 포드 스케일링을 자동으로 해주는것
        - 포드 스케일링 하는 방법은 2가지
            - HPA: 포드 자체를 복제 처리하여 처리할 수 있는 포드의 개수를 늘림(Horizontal)
            - VPA: 리소스를 증가시켜 포드의 사용 가능한 리소스를 늘리는 방법(Vertical) -> 쿠버네티스에서 지원 X, 클라우드에서 제공 해주기도 함
        - 클러스터 자체를 늘리는 방법도 존재
            - CA: 노드를 추가해서 클러스터를 늘리는법, 자동으로 ADD가 불가능하기 때문에 클라우드 에서만 가능 ㅎ
        - HPA는 간단하게 autoscale 명령어를 통해서 스케일링 가능~(레플리카셋이나 디플로이먼트만 가능)
            - 포드 자체는 스케일링이 가능한 구조가 아니기 때문에 레플리카셋이나 디플로이먼트만
            - min(최소 팟 개수), max(최대 팟 개수), cpu-precent(cpu 사용률이 얼마 이상이 되면 스케일링) 등의 설정
        - HPA같은걸 하기전에는 리소스 모니터링할수있는게 필요함(metrics-service 등)

# 쿠버네티스 모니터링 시스템

- 쿠버네티스 모니터링 플랫폼이 다양함
    - Heapster(deprecated됨, Metrics Services로 대체됨)
    - cAdvisor(kubelet에 데이터 통합)
    - 프로메테우스, EFK 등
- 리소스 모니터링 도구
    - 쿠버네티스 클러스터 내에서 애플리케이션 성능을 검사
    - 쿠버네티스는 각 레벨에서 애플리케이션의 리소스 사용량에 대한 상세 정보를 제공
    - 애플리케이션의 성능을 평가하고 병목현상을 제거하여 전체 성능 향상을 모도
    - 리소스 메트릭 파이프라인(쿠버네티스가 기본 제공, 가벼움)
        - Kubectl top 등의 유틸리티 관련 메트릭들로 제한된 팁합을 제공
        - 단기 메모리 저장소인 metrics-server에 의해 수집
        - Metrics-server는 모든 노드를발견하고 kubelet에 CPU와 메모리 질의
        - kubelet은 kubelet에 통합된 cAdvisor를 통해 레거시 도커와 통합후 metric-server 리소스 메트릭으로 노출
    - 완전한 메트릭 파이프라인(프로메테우스 같은거 써야함)
        - 보다 풍부한 메트릭에 접근
        - 똑같이 kubelet을 통해 가져오지만 cAdvisor를 통해 가져오는게 아니라 프로테메우스를 통해서 가져오는것
- 애플리케이션 로그 관리
    - 컨테이너 단위로 로그 확인 가능
    - 싱글컨테이너 포드의 경우 포드까지만 지정해서 로그 확인
    - 멀티컨테이너 포드의 경우 컨테이너 이름까지 전달해서 로그 확인
        - ```Kubectl logs <pot name> <container name>```
    - Kube api가 동작하지 않는 경우 어차피 모든 리소스는 도커를 이용하기때문에, 도커의 로깅 기능을 쓸 수 있음
        - 네트워크, 마스터 서버와 통신 두절, cetificate 만료 등 apiserver가 동작 안할 이유가 많긴함
- Kube Dashboard
    - 쿠버네티스에서 공식적으로 지원함
    - github에서 명령어 하나 따와서 실행하면 됨
    - Kubernetes-dashboard 네임스페이스에서 만들어지는데, 최초 만들어질때 clusterIP로 만들어지기 때문에 노드포트 아니면 로드밸런서 같은걸로 바꿔줘야함
    - 첫 접속시에 토큰을 입력 해야하는데 이는 서비스어카운트에 대한 토큰임
        - 이 토큰에 대한 정보는 시크릿에 있기 때문에, kubernetes-dashboard 네임스페이스에 있는 시크릿에서 Describe 해서 가져오면 됨
    - 근데 들어가도 아직 권한이 없기떄문에 보이는건 없을거임
        - 권한을 주기 위해서는 RBAC라는걸 써야함
        - 원래대로라면 서비스 어카운트를 하나 만들고 클러스터 롤 바인딩을 만들어줘야함
        - 클러스터 롤 바인딩의 서브젝트 부분에서 권한을 줄 서비스 어카운트(kubernetes-dashboard)를 입력해서 잘 주면 확인 가능
    - 모니터링만 가능한게 아니라 편집/삭제도 가능함
- 프로메테우스
    - 쿠버네티크 클러스터에서 메트릭을 자동으로 수집하여 프로메테우스 서버에서 수집하고, 그라파나로 시각화
    - 프로메테우스 아키텍처
        - 외부에서 그라파나 서비스를 통해 그라파나 포드로 접속
        - 그라파나 포드에 pvc가 물려있는데, 거기에 데이터를 제공해주는게 프로메테우스 서비스
        - 프로메테우스 서비스는 프로메테우스 스테이트풀셋을 통해서 모든 소스를 주기적으로 쿼리하여 구성된 모든 데이터를 수집
        - 각 노드별 데몬셋 설치하기도 하고, 쿠버네티스 API를 통해서 가져오기도 하고, 쿠버네티스 메트릭 서비스를 통해서 데이터를 다 가져옴
- EFK
    - ELK에서 logstash 대신에 fluentd를 사용해 수집함
    - 프로메테우스가 메트릭을 수집해 성능을 모니터링하는것과는 다르게 도커의 각 포드의 로그를 수집하고 시각화 및 분석하는 플랫폼을 구축
    - 네임스페이스 별 로그 발생수, 최근 15분 총 로그 발생수, 네임스페이스별 타임시리즈 로그 수, 포드별 로그 발생 수 등등 다양한 정보를 시각화 가능

- 네트워크 메시 환경에서 모니터링 - istio
    - 쿠버네티스 환경에서 다수의 컨테이너의 트래픽을 관찰하고 정상 동작하는지 모니터링 하기 어렵기 떄문에, 이런 서비스 메시 환경에서 확인할 방법이 필요함
        - 서비스 메시: 서비스 메쉬(Service Mesh)란 애플리케이션의 다양한 부분들이 서로 데이터를 공유하는 방식을 제어하는 방법. 서비스 간 커뮤니케이션을 관리하는 다른 시스템들과 달리, 서비스 메쉬는 애플리케이션에 구축된 전용 인프라 계층
    - 이런 서비스 메시 환경에서 대중적으로 쓰이는게 istio임
        - istio는 포드 안에 proxy를 배치해서 proxy를 통해 통신을 하게 함으로써 속도는 조금 느려지지만, 트러블 슈팅에 용이해짐
        - 서비스의 코드를 변경하지 않고 쓸 수 있다는 장점
        - proxy 사이드카가 배치되서 걔를 통해서 통신하기 때문에 서비스의 코드가 변경되지 않음
        - 네임스페이스에 대한 정의나 레이블만 바꿔주면 알아서 적용되니까 편리함
    - 쿠버네티스의 복잡성을 감소 시킬수있음
    
# 클러스터 유지 & 보안
- 노드 유지보수
    - 커널 업그레이드, libc 업그레이드, 하드웨어 복구 등 노드를 재부팅 해야 하는경우
    - 노드를 갑자기 내리면 문제가 발생 할 수 있음
    - 노드 업그레이드를 효율적으로 하려면 3가지 절차를 거쳐야함
        - Drain node: 노드에 있는 팟을 내리는 절차
        - Upgrade node: 노드 업그레이드
        - Uncordon node: drain node를 하면 노드에 있는 모든 팟을 내보내고, cordon되어서 팟들이 들어오지 못하게 함. 따라서 이걸 해제 해줘야함. 스케줄링에서 제외 되어 버리는것
        - uncordon을 한다고 하더라도 팟이 되돌아오지는 않음. Pending 되었던거나 deamon set과 같은 애들은 자동으로 배치가 되지만, 한번 다른 노드로 넘어간 팟들이 돌아 오지는 않음
- 쿠버네티스 버전 업그레이드
    - 패키지마다 모든 컴포넌트는 동일한 버전으로 배포된다
        - API서버, 컨트롤러 매니저, 스케줄러, 큐블렛, 큐브프록시 등
        - ETCD, 네트워크, CoreDNS는 제외
    - 쿠버네티스는 apiserver를 기준으로 호환성을 제공
        - apiserver + 나머지: x ~ x-1 ( 1.13 ~ 1.12)
        - Kubelet: x ~ x-2
        - Kubectl: x + 1 ~ x - 1
    - 쿠버네티스 버전 업그레이드가 필요한 경우, 노드 OS업그레이드 하는것처럼 하나씩 drain하면서 버전을 업데이트 하는방법을 사용
- 백업과 복원
    - 백업 리소스의 종류
        - Get all 같은거 하면 나오는 모든 pod에 대한 데이터
        - 부가데이터 (configmap 같은) 건 etcd 디비를 백업 해야함
        - Peristent volume은 일반적인 방법으로 백업
        - Etcd 백업은 조금 독특함. 기존 데이터 수정 안하고 복원된 데이터만 따로 뽑아낼 수 있음
            - Etcd도 클러스터로 구성할수 있으니 고려해야함
- 보안을 위한 다양한 리소스
    - 모든 통신은 TLS
        - 대부분의 엑세스는 Kube-apiserver를 통해서만 가능하다
    - 엑세스 가능한 유저
        - CSV파일로 유저 만들기 - 유저이름과 토큰 기록해서 Kube-apiserver가 시작할때 읽어서 만드러줌. 따라서 적용하려면 kube-apiserver가 재시작 되어야함
        - Service Account
        - 인증서
        - External Authentication Provider - LDAP 등
    - 뭘 할수있는가
        - RBAC Authorization
        - ABAC Authorization
        - Node Authorization
...
- 어카운트
    - 2가지 타입이 존재
        - Kubernetes 클러스터를 사용하는 사용자를 위한 user
        - 어플리케이션을 위한 service account
    - 스태틱토큰
        - 유저를 만드는 가장 쉬운 방법
        - Apiserver를 시작할때 —token-auth-file에 csv파일이 전달되면 그걸로 적용됨
        - 토큰, 사용자이름, 사용자uid, 그룹(선택적)으로 구성
        - apiserver에 등록되서 바로 실행되면 바로 적용되므로, http 요청시 헤더에 Authorization 헤더를 추가 해야함
        - Kubectl config set-credentials 해서 kubectl에 추가 시켜두는 방법도 있음
            - Set-context를 통해서 컨텍스트도 같이 지정 해야함
    - 서비스 어카운트
        - 기본 서비스 어카운트는 생성됨(default)
        - 명령어를 사용해서 서비스 어카운트를 만들 수도있음
- TLS 인증서를 활용한 통신
    - SSL/TLS통신은 응용계층인 HTTP와 TCP사이에서 작동(OSI 5 ~ 6)
        - 어플리케이션에 독립적
        - 데이터의 암호화, 무결성, 서버인증기능, 클라인증기능
    - HTTPS = HTTP + SSL
    - HTTPS
        - HTTPS를 열기 위해선 인증서가 필요함
        - 서버에서 공개키(CSI)를 만들고 인증 기관을 찾아야함
        - 클라에서 요청을 하면 서버의 퍼블릭 키와 인증 정보를 넘겨주고, 클라가 ca기관으로부터 인증을 확인
        - 클라에서의 확인이끝나면 클라에서 세션키(공개키는 너무 기니까, 앞으로 쓸 키를 새로 만든것)를 서버로 보냄
        - 서버에서도 세션키를 주고 받으면서 연결이 시작되는것임
        - 인증서는 1년짜리니까 갱신 해야대~
    - 쿠버네티스에서 인증서 위치
        - /etc/kubernetes/pki
    - 정확한 TLS인증서를 사용하고 있는지 점검
        - manifest파일에서 어떤 인증서를 쓰는지 확인
    - 인증서 정보 확인하기
        - openssl을 통해서 확인
        - issuer가 중요함, issuer는 인증을 해준 기관, subject는 인증을 받은 사람
    - 인증서 자동으로 갱신하는 방법
        - Check certificate expiration: kubeadm alpha cert check-expiration
        - Automatic certificate renewal: 컨트롤 플레인을 업그레이드 하면 모든 인증서가 자동으로 갱신됨
        - Manual certificate renewal: kubeadm alpha certs renew all
    - 쿠버네티스 인증서는 기본 1년, CA는 10년이다~
- TLS인증서를 활용한 유저 생성
    - ca를 사용해서 직접 csr승인하기
        - 개인키 생성
        - private키를 기반으로 인증서 서명 요청하기
        - Kubernetes 클러스터 인증기관(CA) 이 요청을 승인해야함
            - 내부에서 직접 승인하는 경우 ca.key와 ca.crt를 사용하여 승인 가능
- Kube config를 통해서 인증
    - Kube config는 3가지 부분으로 이뤄져있다
        - Clusters: 연결할 쿠버네티스의 클러스터정보입력
        - Contexts: cluster와 user를 함꼐 입력하여 권한 할당
        - Users: 사용할 권한을 가진 사용자 입력
        - 각각의 user와 cluster가 잘 맞는지 확인 해야함
- RBAC
    - 기업내에서 개별 사용자의 역할을 기반으로 컴퓨터, 네트워크 리소스에 대한 엑세스를 제어
    - rbac.authorization.k8s.io API를 사용하여 정의
    - 권한 결정을 내리고 kubernetes api를 통해 정책을 동적으로 수정
    - apiserver에 —authorization-mode=RBAC 옵션이 필요
    - RBAC를 다루는 API는 4가지 리소스를 컨트롤
        - 롤/ 롤바인딩 -> 네임스페이스에 종속적. 일반 사용자들에게 뿌림
        - 클러스터 롤/ 클러스터 롤바인딩 -> 클러스터 전체에서 사용됨. 네임스페이스에 구애받지 않음, 관리자들에게 주어짐
    - 롤 / 롤바인딩
        - 롤: 누가 하는지 정하지 않고, 무엇을 할지만 정함
        - 롤 바인딩: 누가하는지만 정하고, 무엇을 하는지는 정하지 않음
- 시큐리티 콘텍스트
    - 권한을 축소시켜서 보안취약점을 감소시키기 위해서 사용
    - 시큐리티 컨텍스트는 다음 사항을 포함
        - 권한 상승 가능여부
        - 프로세스 UID/GID를 활용한 파일등의 오브젝트 엑세스 제어
        - Linux Capabulities를 활용한 커널 기능 추가
        - 오브젝트에 보안 레이블을 지정하는 SELinux 기능
    ...
    - 포드에 시큐리티 컨텍스트 설정하기
        ...
        - 컨테이너 내부에 새로운 유저를 사용하면 그 설정이 우선시됨(Pod설정이 default고 container의 설정이 override 되는듯)
        - capabilities를 적용하면 리눅스 커널에서 사용할 수 있는 권한을 추가 설정 가능
    - SELinux
        - 모든 프로세스와 객체에 시큐리티 콘텍스트 혹은 레이블을 달아 관리하는 형태
        - 복잡하기때문에 실무에서 잘 쓰지는 않음
- 네트워크 정책
    - 방화벽같은것
    - 팟들끼리의 통신을 제한
    - CNI플러그인의 종류에 따라서 되고 안되고 나뉨
    - 포드그룹이 서로 및 다른 네트워크 엔드포인트와 통신하는 방법을 지정
    - Network Policy 리소스는 레이블을 사용하여 포드를 선택
    - 선택한 포드에 허용되는 트래픽을 지정하는 규칙을 정의
    - 특정 포드를 선택하는 네임스페이스에 Network Policy가 있으면 해당 포드는 Network Policy에서 허용하지 않는 요청을 거부
    - Egress
        - 나가는 트래픽에 대한 정책 설정
        - ipBlock 을 통해 나가려는 하는 ip대역 설정
        - Ports에 어떤 포트를 허용하고자 하는지 명시
        - 기본 설정을 하는순간 default deny가 되어서 나가는게 다 막히고, 그 이후에 white list를 정의
    - Ingress
        - 들어오는 트래픽에대한 설정
        - Ipblock을 통해 들어오는 ip 대역 설정
        - except를 통해 예외도 가능
        - 기본 설정을 하는순간 default deny가 되어서 나가는게 다 막히고, 그 이후에 white list를 정의
        - Ipblock, namespceSelector, podSelector등의 조건이 있는데 모두 or조건이라 셋중 하나만 만족해도 들어와짐
    - 기본적으로 정책들은 or연산이라 하나라도 허용이 되면 허용되어버림~
- 클러스터, 어플리케이션 트러블슈팅 가이드
    - 로깅 잘하고, 로깅 해석 잘하고, 모니터링 잘 하는게 어쨋건 베스트
    - 어플리케이션 자체의 문제인지 클러스터의 문제인지 확인을 해야함
    - 어플리케이션 트러블슈팅
        - 각각의 포드, 컨피그맵등 로직에서 무엇이 잘못됫는지
        - 일반적으로 이름이나 레이블링이 다른데서 발생
    - 클러스터 트러블슈팅
        - 쿠버네티스 시스템애 대한 깊은 이해를 요구
        - 다수에서 발생하기보단 한두군데 에서 문제가 발생
        - kubectl이 접속되지 않으면 service api 자체의 문제일 가능성이큼
        - 컨테이너가 장시간 배치가 안되면 스케줄러문제
        - 특정 노드나 포트가 연결이 안되면 kubelect 확인
        - journalctl을 사용해서 동작하지 않는 노드의 데이터 확인
