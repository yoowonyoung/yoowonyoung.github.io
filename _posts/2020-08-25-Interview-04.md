...
- 주요 사용 기술
    ...
    * Kubernetes
        ...
        - 자동화된 롤아웃, 롤백이 지원되며 Horizontal 스케일링 및 로드밸런싱 기능등이 있ㅇㅁ
    * Docker
    ...
    * 구성
        ...
        - 빌드된 docker image를 사내 dockerhub인 d2hub로 push
        - Jenkins Agent가 helm install을 실행해서 사내 chart repository에서 chart를 받아 클러스터에 설치
        - chart에 명시되어있는 docker image를 d2hub에서 받아 pod, service, ingress를 클러스터에 배포

- 역할
    * 기존 SandBox, CBT Phase 에만 적용 되어있는, Jenkins + Kubernetes 를 Dev Phase로 확장
...
- 역할
    ...
    * CROWD SSO를 이용한 Login 기능 구현
        + 회사에서 쓰이는 계정 종류가 다양했는데(knox, AD 등) 가장 많은곳(JIRA/Confluence등 )에서 사용되고있던 계정이 AD계정임에 착안, CROWD SSO를 구현함으로써 해당 서비스에만 로그인해도 우리 서비스에 로그인이 가능 하도록 구현
        + FE단에서 넘겨주는 토큰을 활용해서 해당 서비스에 로그인되있는 유저는 바로 로그인이 되도록 했음
        + 추후 Knox SSO도 구현함
